...
//			esc_html__( 'Master Addons', MELA_TD ),    // Menu Title
	...
	public function master_addons_el_admin_scripts( $hook ) {
		...
		wp_enqueue_style( 'master-addons-notice-css', plugins_url( '/', __FILE__ ).'assets/css/master-addons-notice.css' );
		if( isset( $hook ) && $hook == 'elementor_page_master-addons-settings' ) {
			wp_enqueue_style( 'master-addons-el-admin', plugins_url( '/', __FILE__ ).'assets/css/master-addons-admin.css' );
			wp_enqueue_script( 'master-addons-el-admin', plugins_url( '/', __FILE__ ).'assets/js/master-addons-admin.js',
				array(
				'jquery'),
				'1.0', true );
		}
